"""
UR5 Simulation Client for OpenPI Policy Server

This script runs a UR5 simulation environment and connects to the OpenPI policy server
for action prediction. Adapted from examples/libero/main.py for UR5 robot.

Usage:
    # Start policy server in Terminal 1:
    uv run scripts/serve_policy.py --env=DROID
    
    # Run this client in Terminal 2:
    python examples/ur5/ur5_main.py --prompt "pick up the red cube"
"""
...
class Args:
    ...
    # Task specification
    #################################################################################################################
    prompt: str = "pick up the red cube"  # Language instruction for the robot
...
class UR5Env:
    ...
    def __init__(self, scene_xml: str, camera_width: int = 224, camera_height: int = 224, seed: int = 42):
        ...
        self.model = mujoco.MjModel.from_xml_file(scene_xml)
        ...
        logging.info(f"UR5 Environment initialized:")
    ...
    def close(self):
        """Clean up resources."""
        # Renderer cleanup is automatic in newer mujoco versions
        pass


def run_ur5_simulation(args: Args) -> None:
    """Main function to run UR5 simulation with policy server."""
    ...
    logging.info("=" * 60)
    logging.info("UR5 Simulation with OpenPI Policy Server")
    logging.info("=" * 60)
    ...
    for trial_idx in range(args.num_trials):
        ...
        while t < args.max_steps + args.num_steps_wait:
            try:
                ...
                if t % 20 == 0:
                    logging.info(f"Step {t}/{args.max_steps}: Action executed")
        ...
        if replay_images:
            video_filename = f"ur5_trial_{trial_idx}_{args.prompt.replace(' ', '_')[:30]}.mp4"
